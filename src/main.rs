...
impl MerkleTree {
    ...
    pub fn new<T: Hash>(data: Vec<T>) -> MerkleTree {
        ...
        let mut tree = MerkleTree {
            root: hashes[0].clone(),
            layers: MerkleTree::create_layers(hashes),
            unique_elements: last_elem_position,
        };

        if last_elem_position > 1 {
            tree.root = hash(
                &((*tree.layers.last().unwrap())[0] as u128
                    + (*tree.layers.last().unwrap())[1] as u128),
            );
        }
        tree
    }
    ...
    pub fn add_element<T: Hash>(&mut self, elem: T) {
        ...
        if !repeats_present {
            ...
            let hash_pre_root_left = hash(
                &(self.layers.last().unwrap()[0] as u128 + self.layers.last().unwrap()[1] as u128),
            );
            let hash_pre_root_right = hash(
                &(self.layers.last().unwrap()[2] as u128 + self.layers.last().unwrap()[3] as u128),
            );
            ...
            self.root =
                hash(&(last_layer_before_root[0] as u128 + last_layer_before_root[1] as u128));
        } else {
            ...
            for i in 1..self.layers.len() {
                let new_hash = hash(
                    &(self.layers[i - 1][index_to_be_updated * 2] as u128
                        + self.layers[i - 1][index_to_be_updated * 2 + 1] as u128),
                );
    ...
    fn create_layers(hashes: Vec<HashValue>) -> Vec<Vec<HashValue>> {
        ...
        for _i in 0..layers_amount {
            ...
            for j in 0..(current_layer.len() / 2) {
                let new_hash =
                    hash(&(current_layer[j * 2] as u128 + current_layer[j * 2 + 1] as u128));
        ...
        for i in 0..merkle_tree.layers.len() {
            ...
            for j in 0..current_layer.len() / 2 {
                next_layer.push(hash(
                    &(current_layer[j * 2] as u128 + current_layer[j * 2 + 1] as u128),
                ));
            }
